<template>
  <div :class="$style.container" @click="popup = true">
    <div :class="$style.text">
      <div :class="$style.name">Медный Купорос</div>
      <div v-if="$props.status === 2" :class="$style.description">O.O.O.O.O.[O-]S(=O) (=O)[O-].[Cu+2]</div>
    </div>
        ...
        <div :class="$style.popupContainer" @click.stop>
          ...
          <template v-if="$props.status === 2">
            ...
            <div :class="$style.info">
              Медный Купорос (CuSO<sub>2</sub>*<sub>5</sub>H<sub>2</sub>O) <br/>
              – Синий цвет<br/>
              – При нагревании выделяет воду<br/>
            </div>
          </template>
          <template v-else>
